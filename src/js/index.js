$(document).ready(function () {
    // If you want to add a new stack of flash cards, add them here and link to the arrays in selectCategories()
    ...
    var chessCategories = ['ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics','ChessTactics'];
    var chessFront = ['Direct Attack tactics','Fork','Pin','Skewer','Discovered Attack','Hanging Pieces','Breaking Defense tactics','Overloading','Undermining','Deflection','Decoy','Pawn break through','Combo tactics','Sacrifice','Intermezzo','X-Ray attack','Battery','Rare but powerful tactics','Piece trap','Interference','Windmill','Desperado','Endgame win tactics','Simplification','Zugzwang (move compulsion)','Smothered Mate','Underpromotion','Endgame forced draw tactics','Forced stalemate','Perpetual check','Fortress'];
    var chessBack = ['1. Fork, 2. Pin, 3. Skewer, 4. Discovered Attack, 5. Hanging Pieces','One piece attacks two or more enemy pieces simultaneously','A piece is unable to move because doing so would expose a more valuable piece (or the King) behind it (only bishops/rooks/queens can do this)','A piece attacks a valuable enemy piece, forcing it to move and exposing a less valuable piece behind it; pin has more valuable piece behind less valuable one; skewer is the reverse; only bishops/rooks/queens can do this','Moving a piece to unblock a line of attack for a friendly piece standing behind it. Also includes Discovered Check and Double Check','Leaving a piece unprotected and exposed to capture','1. Overloading, 2. Undermining, 3. Deflection, 4. Decoy, 5. Pawn break through','Targeting a defensive piece that is trying to protect too many squares or pieces at once','Capturing or driving away a defending piece to leave an enemy target vulnerable (also called Removing the Defender)','Forcing a key defender to move away from its job','Forcing a piece to a bad or poison square','tactical pawn push, often involving a sacrifice, to shatter an opponents pawn structure, open lines, create weaknesses, or, most importantly, forge a powerful passed pawn that can march towards promotion','1. Sacrifice, 2. Intermezzo, 3. X-Ray attack, 4. Battery','Deliberately giving up material to gain a tactical or positional advantage (e.g., to clear a square or open a line); Includes Queen Sacrifice,  Exchange Sacrifice, and Clearance Sacrifice','Playing a surprising in-between move that creates a new threat before responding to the opponents previous move','Attacking through a piece to pressure or target what lies behind it; only bishops/rooks/queens can do this','Lining up two pieces in a row (bishop, rook, or queen) to form an attack across a line or diagonal ','1. Piece trap, 2. Interference, 3. Windmill, 4. Desperado','Forcing a piece into inescapable capture','Placing a piece to block an enemy line of defense or communication','A repeated series of discovered checks that lets you capture material between checks; only bishops/rooks/queens can do this','Using a piece that will be lost to try to capture the best opponent piece along the way','1. Simplification, 2. Move compulsion, 3. Smothered Mate, 4. Underpromotion','When winning, trade pieces with opponent to avoid complicated games that could lead to mistakes','Any move worsens your position because you are forced to move','Checkmate delivered when the king is trapped by its own pieces, usually via a knight','Promoting a pawn to a piece other than a queen for tactical reasons','1. Forced stalemate, 2. Perpetual check, 3. Fortress','Deliberately engineering a position with no legal moves but not in check','Checking repeatedly so the opponent cannot escape, forcing a draw','Create a protected wall so opponent cant penetrate without a sacrifice',];
    ...
    function selectCategories() {
        ...
        if (page == "spanish.html") {
            ...
            dictionaryCat = chessCategories;
            englishDictionary = chessFront;
            foreignDictionary = chessBack;                 
        }
