 ...
 * Table: `group_members` — `group_handle` (matches `groups.handle`), person_handle, status (no `group_name`; title lives on `groups` only)
 *
 ...
 *     group_handle text not null references public.groups(handle),
...
/** Stored `groups.handle` / `group_members.group_handle` — no `@`, lowercased. */
...
async function listGroupsForUser(userId) {
  ...
  const gdRes = await supabase
    ...
    .select('group_name, handle')
    .in('handle', distinctHandles);
  if (!gdRes.error && gdRes.data) {
    for (const r of gdRes.data) {
      const gn = r.group_name != null ? String(r.group_name).trim() : '';
      const h = r.handle != null ? String(r.handle).trim() : '';
      if (gn && h && !handleMapByGn[gn]) handleMapByGn[gn] = h;
    }
  } else if (gdRes.error) {
    const msg = String(gdRes.error.message || '').toLowerCase();
    const missingRel =
      (msg.includes('group_details') || msg.includes('schema cache')) &&
      (msg.includes('does not exist') || msg.includes('could not find') || gdRes.error.code === '42P01');
    if (!missingRel) throw gdRes.error;
  }
...
/**
 * All `group_members` for this group (`group_handle` matches URL / `groups.handle`).
